...
# ---------------------------------------------------------------------------
# FIPS state-code decoding (Bugwood "Location (State)" column = "us_<FIPS>")
# ---------------------------------------------------------------------------
#
# The Bugwood feed stores the state in the ``Location (State)`` column as a
# coded token ``us_<n>`` where ``n`` is the ANSI/FIPS state numeric code
# (e.g. ``us_1`` = Alabama, ``us_37`` = North Carolina). The adjacent plain
# ``Location`` column carries the human-readable name, but ``decode_state``
# lets any consumer recover the canonical name from the code alone.
...
STATE_FIPS: dict = {
    ...
    72: "Puerto Rico", 66: "Guam", 78: "US Virgin Islands",
}


def decode_state(code: Optional[str]) -> Optional[str]:
    """Decode a Bugwood ``us_<FIPS>`` token to a canonical US state name.

    Accepts ``"us_37"``, ``"US_37"``, ``"37"``, or an already-decoded name
    (returned unchanged if recognised). Returns ``None`` for unknown codes.
    """
    if not code:
        return None
    s = str(code).strip()
    ...
    if s.lower() in US_STATE_CENTROID:
        return s.title() if s.islower() else s
    digits = s.lower().replace("us_", "").lstrip("us").strip(" _")
    if digits.isdigit():
        return STATE_FIPS.get(int(digits))
    return None
